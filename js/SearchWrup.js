    ...
    '0': function (require, module, exports, global) {
        ...
        var Main = prime({
                ...
                displayData: function () {
                    var container = $$('#data');
                    // console.log("container ::>>>>>>>>>>>>>>>>>>>>>",fsData)
                    Arr.forEach(fsData, function (data) {
                        zen('li').text(data).insert(container);
                    });
                },
                displayResults: function (results, container, flag) {
                    ...
                    if(flag){
                        ...
                        var ar =[];
                        results.forEach(function(a,inx){
                            var obj = {}
                            obj["data"] = a.value;
                            obj["score"] = number.round(a.score);
                            ar.push(obj);
                        });

                        console.log("finArr B >>>>>>>> ",ar)

                        Arr.forEach(ar, function (r,ind) {
                            zen('li').text(r.data + ' (total: ' + r.score + ')').insert(container);
                        });
                    }else{
                        console.log("results >>>>>>>>>>>>>>",results)
                        var ar =[];
                        results.forEach(function(a,inx){
                            var obj = {}
                            obj["data"] = a.replace(/[\#\_]/g," ");
                            // obj["score"] = 0;
                            ar.push(obj);
                        });
                        Arr.forEach(ar, function (r,ind) {
                            zen('li').text(r.data).insert(container);
                        });
                    }
                    
                },
                search: function () {
                    /*Logic Updated By Kinj*/
                    var term = this.searchField.value();
                    term = term.toLowerCase();
                    // term = term.replace(/([a-z])\1+/g, "$1");
                    var fullData = fsData.join('#') + '#';
                    fullData = fullData.replace(/\s/g,"_").toLowerCase();
                    console.log("term ::>>>>>>>>>>>>>> ",term)
                    // console.log("CONDI ::>>>>>>>>>>>>>> ",fullData.match(new RegExp('(?!#)(\\w*)('+term+')(\\w*)(?=#)','gi')))
                    if(fullData.match(new RegExp('(?!#)(\\w*)('+term+')(\\w*)(?=#)','gi'))){
                        var levRes = fullData.match(new RegExp('(?!#)(\\w*)('+term+')(\\w*)(?=#)','gi'));
                        console.log(".......................",levRes);
                        this.displayResults(levRes, $$('#results'), false);
                    }else{
                        var term1, term2;
                        term = term.replace(/([^edt])\1+/g, "$1");
                        console.log("levResults 111::>>>>>>>>>>>>>>1111 ",term)

                        if(term.match(/([dt])\1+/g)){
                            term1 = term;
                            term2 = term.replace(/([dt])\1+/g, "$1");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/([dt])/g)){
                            term1 = term;
                            term2 = term.replace(/([dt])/g, "$1$1");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/([a-z])(ee)([a-z])/ig)){
                            term1 = term;
                            term2 = term.replace(/([a-z])(ee)([a-z])/ig,"$1i$3").replace(/([a-z])(ee)([a-z])/ig,"$1i$3");  //.replace(/([a-z])(i)([a-z])/g,"$1ee$3");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/([o])(h)/ig)){
                            term1 = term
                            term2 = term.replace(/(h)/ig,"").replace(/(h)/ig,"");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/([o])/ig)){
                            term1 = term
                            term2 = term.replace(/([o])/ig,"$1h");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.charAt(0).toLowerCase() == 'u'){
                            term1 = term
                            term2 = term.replace(/^u/i,"o");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2);
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.charAt(0).toLowerCase() == 'o'){
                            term1 = term
                            term2 = term.replace(/^o/i,"u");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2);
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/([a-z])(sh)([a-z])/ig)){
                            term1 = term
                            term2 = term.replace(/([a-z])(sh)([a-z])/g,"$1s$3").replace(/([a-z])(sh)([a-z])/g,"$1s$3");//.replace(/([a-z])(s)([a-z])/g,"$1sh$3");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/([a-z])(s)([^h])/ig)){
                            term1 = term
                            term2 = term.replace(/([a-z])(s)([a-z])/g,"$1sh$3").replace(/([a-z])(s)([a-z])/g,"$1sh$3");//.replace(/([a-z])(s)([a-z])/g,"$1sh$3");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/i$/ig)){
                            term1 = term
                            term2 = term.replace(/i$/g,"y");//.replace(/([a-z])(s)([a-z])/g,"$1sh$3");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/y$/ig)){
                            term1 = term
                            term2 = term.replace(/y$/g,"i");//.replace(/([a-z])(s)([a-z])/g,"$1sh$3");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/^v/ig)){
                            term1 = term
                            term2 = term.replace(/^v/i,"w");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2);
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/^w/ig)){
                            term1 = term
                            term2 = term.replace(/^w/i,"v");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2);
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/([kgcjtdpbr])(h)/ig)){
                            term1 = term
                            term2 = term.replace(/(h)/ig,"").replace(/(h)/ig,"");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                        }else if(term.match(/([kgcjtdpbr])/ig)){
                            term1 = term
                            term2 = term.replace(/([kgcjtdpbr])/ig,"$1h");
                            console.log('levResults ::>>>>>>>>>>>>>>> ',term1,term2)
                            var levResults = this.fuzzySearch.search(term1);
                            var levResults1 = this.fuzzySearch.search(term2);
                            (levResults[0].score >= levResults1[0].score) ? this.displayResults(levResults, $$('#results'), true) : this.displayResults(levResults1, $$('#results'), true);
                            ...
                            this.displayResults(levResults, $$('#results'), true)
                        }
                    }
                }
            });
    ...
    'q': function (require, module, exports, global) {
        ...
        var FuzzySearch = prime({
                ...
                search: function (needle) {
                    // console.log("needleneedle :>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>",needle)
